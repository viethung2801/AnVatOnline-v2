...
//    where od.order_id in (select id from orders o where o.state = 0 and cast(o.created_date as date) = '2023-11-17')
    @Query("select sum((od.quantity * p.price)-(od.quantity * p.cost)),sum(od.quantity * p.price)" +
            "from OrderDetail od " +
            "join Product p on od.product.id = p.id " +
            "where od.order.id in (select o.id from Order o where o.state = 0 and cast(o.createdDate as localdate ) = :date)")//state = 0 =>success
